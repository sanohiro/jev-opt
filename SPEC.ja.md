...
実験の問いは **「人または Claude がマークした関数を、Jev がヒントで最大限速くできるか」** の一点である。v0.3(ループヒントのグローバル掃引)と v0.4(PGO 訓練データ選定・FP 再結合)は git 履歴(`git show c58545d:SPEC.ja.md` が v0.4、その中の参照 `3a41f49` が v0.3)と `docs/decisions.ja.md` に残っており、否定結果と経緯はそちらを参照する。v0.5 は決定 58 の 4 項目にそのまま戻したもので、それ以外は仕様から落とす。本文は決定 66〜89 を反映済み(語彙 v4、state v4.1、読み出し規則、採用規則、hintbench の実測、探索機構(予定)、対象の順序)。決定 60・62・75〜78 は実装状況として参照する。決定事項の一覧表はこの仕様には置かず `docs/decisions.ja.md` を正本とする。方針は 4 つ: **速さは追求する / 作りはシンプルにする / 判断は Jev に任せる / わかりやすくする。** 最適化は対象のソースを書き換えずに行う。本文中の主張は **実測 / 仮説(ソース確認のみ)/ 未測定** を明示する。
...
| 種別 | ヒント | 値域 | 適用先 |
|---|---|---|---|
| 関数属性 | `inline(always)` / `inline(never)` / `align=N`(**`align` は外す予定**) | N ∈ {16, 32, 64} | マークされた関数(LLVM 属性 `alwaysinline` / `noinline` / `align`) |
| ループ | `unroll.count=N` / `unroll.disable`(**`interleave.count=1` と統合予定**) | N ∈ {2, 4, 8} | マーク内の各ループ(`llvm.loop.*` metadata) |
...
| 指標 | 定義 | 値 |
|---|---|---|
| **主指標** | Jev の最終 plan − 基準(集計速度比、95% 信頼区間つき) | hintbench **+6.3〜+6.8%**(実測、決定 88)。jaq TBD |
...
| 補助 | Jev vs ランダム探索(同ラウンド数、同 site 集合) | hintbench: 採用ラウンド 2 対 **0**、有害選択 2(いずれも 1 ラウンドで撤回)対 **5**(撤回なし) |
...
- **ノイズは対象ごとに桁が違う(実測)。** hintbench の null パネルは**バッチ内 0.08 pt、バッチ間 0.17 pt**で、jaq の **0.77 / 4.67 pt** よりはるかに静か。この差は測定手順ではなく**対象の性質**と読む(hintbench の 1 因子 arm はコードを測っており、jaq のそれは機械を測っていた。oracle §7。**仮説**)ので、jaq の較正不良を hintbench の結果に持ち込まない(逆も同じ)。対象ごとに null パネルを回してから採否の規則を当てる。
...
5. **探索機構の実装** — 決定 89(a)〜(d) と 87(c)(§1 (3))。未試行候補の Choice、plan 変更時のみの採用、履歴の他ケースコストと共有ケース明記、plugin のベクトル化後事実。
6. **hintbench 再走** — 同じ凍結条件で、探索機構が **oracle の組み合わせにどこまで届くか**(現状 71〜77%)を測る。取りこぼし 2 件(k3 ループ `unroll.count=4`、k8 ループ 幅 16)が拾えるかが判定。
7. **jaq の関数属性 oracle を再実行** — v1 で走らせた 90 arm はフラットだった(決定 79)が、1 Choice が 60 クロージャに波及していた(決定 80(c) の修正で 138 → 49 entry)。**v4 + クロージャ修正で再実行(進行中)。**
...
10. **zopfli を転移先として通す** — hintbench / jaq で凍結した手順をそのまま当てる。per-case の読み出しが無い対象(`own_workload_of` が `None`)なので、site の履歴は集約比だけになる。
11. **記事**(zenn)。

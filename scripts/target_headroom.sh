#!/usr/bin/env bash
#
# Headroom sweep on a target's PGO baseline (SPEC.ja.md 6.3, 13 day-0 item 5).
#
# Every configuration is the PGO baseline plus one (rarely two) -Cllvm-args
# knobs: same merged.profdata, same debuginfo=1, same panic strategy, same
# remark flags, explicit --target, its own CARGO_TARGET_DIR, clean build.
#
# Primary read is the remark diff (SPEC.ja.md 6.3): dump the sorted `remark:`
# lines of the build log and compare to the baseline's. Alongside it, the
# `.text` hash --- results.md Day 0 section 17 found it the sharper skip
# criterion, because a configuration whose `.text` is bit-identical to the
# baseline's is the same machine code and cannot differ in time, whatever its
# remarks say. Timing therefore runs for every configuration whose `.text`
# changed, plus group 0 (the in-sweep noise probe).
#
# Correctness comes before any of it: run_correctness hashes every case's
# output for every configuration, and a configuration whose output differs is
# recorded as a correctness violation and excluded from the headroom
# judgement (results.md Day 0 section 13: the toy's largest measured
# "speedup" was a binary computing a different answer).
#
# Phases:
#   build   build every configuration, collect checksums, .text hashes and
#           remark diffs                                 (BUILD=0 to skip)
#   time    one interleaved bench.py run over the baseline plus every
#           configuration that changed something         (TIME=0 to skip)
#
# Usage:  TARGET=zopfli scripts/target_headroom.sh
#         BUILD=0 scripts/target_headroom.sh       # re-time only
#         MDE=0.031 scripts/target_headroom.sh     # override the frozen MDE
set -euo pipefail

source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/target_common.sh"

# SUITE names this sweep's output directory, so a follow-up probe can run
# without overwriting the main sweep's binaries and remark sets.
# ONLY is an extended regex; when set, only matching configuration names are
# built and analysed (the baseline is always built).
SUITE="${SUITE:-headroom}"
ONLY="${ONLY:-}"
OUT="$REPO/artifacts/$TARGET-$SUITE"
SWEEP_TD="$REPO/target-$TARGET-$SUITE"
BUILD="${BUILD:-1}"
TIME="${TIME:-1}"
RUNS="${RUNS:-10}"
WARMUP="${WARMUP:-3}"
AA_STATS="$REPO/artifacts/$TARGET-aa/aa-stats.json"
BENCH_OUT_NAME="${BENCH_OUT_NAME:-$SUITE}"

# The sweep. One knob per configuration except where a combination is the
# point. Knob names and enum values come from llvm-knobs.json, generated by
# scripts/gen_llvm_knobs.sh from the pinned toolchain; values that are already
# the compiler default are not swept (the baseline is that configuration).
#
# SPEC.ja.md 6.3 group 1 lists `-prefer-predicate-over-epilogue`. That cl::opt
# does not exist on LLVM 23.1.1. Its 23.1.1 successors are used instead:
# -epilogue-tail-folding-policy and -force-tail-folding-style.
CONFIGS=(
  # group 0: noise probe, no decision content
  "g0-align5|-align-all-nofallthru-blocks=5"
  # group 1: vectorizer
  "g1-maxbw|-vectorizer-maximize-bandwidth"
  "g1-vw8|-force-vector-width=8"
  "g1-vw16|-force-vector-width=16"
  "g1-vw32|-force-vector-width=32"
  "g1-maxbw-vw32|-vectorizer-maximize-bandwidth -force-vector-width=32"
  "g1-ic1|-force-vector-interleave=1"
  "g1-ic2|-force-vector-interleave=2"
  "g1-ic4|-force-vector-interleave=4"
  "g1-tailfold-prefer|-epilogue-tail-folding-policy=prefer-fold-tail"
  "g1-tfstyle-data|-force-tail-folding-style=data"
  "g1-tfstyle-data-and-control|-force-tail-folding-style=data-and-control"
  "g1-memcheck24|-runtime-memory-check-threshold=24"
  "g1-memcheck128|-runtime-memory-check-threshold=128"
  # -enable-early-exit-vectorization is NOT swept: results.md Day 0 section 12
  # measured it default-on on LLVM 23.1.1, so passing it changes nothing.
  # group 2: inlining and unrolling
  "g2-inline325|-inline-threshold=325"
  "g2-inline500|-inline-threshold=500"
  "g2-inline1000|-inline-threshold=1000"
  "g2-unroll-thr300|-unroll-threshold=300"
  "g2-unroll-thr1000|-unroll-threshold=1000"
  "g2-unroll-max2|-unroll-max-count=2"
  "g2-unroll-max8|-unroll-max-count=8"
  "g2-unroll-runtime|-unroll-runtime"
  # group 3: the rest
  "g3-loop-distribute|-enable-loop-distribute"
  "g3-slp-neg20|-slp-threshold=-20"
  "g3-slp-100|-slp-threshold=100"
  "g3-unswitch200|-unswitch-threshold=200"
  "g3-loop-flatten|-enable-loop-flatten"
  "g3-gvn-hoist|-enable-gvn-hoist"
  # group 4: ISA and vector-width preference. These are rustc -C flags, not
  # cl::opts; build_variant passes anything starting with -C through as is.
  "g4-prefer256|-Ctarget-feature=+prefer-256-bit"
  "g4-prefer128|-Ctarget-feature=+prefer-128-bit"
  "g4-v3|-Ctarget-cpu=x86-64-v3"
  # group 5: the unroll dimension in detail. Added after the zopfli sweep
  # found -unroll-max-count=2 to be the only knob with a real effect
  # (results.md section 31). -unroll-count forces a count on every loop;
  # -unroll-max-count caps what the heuristic may choose.
  "g5-count2|-unroll-count=2"
  "g5-count4|-unroll-count=4"
  "g5-count8|-unroll-count=8"
  "g5-max1|-unroll-max-count=1"
  "g5-max4|-unroll-max-count=4"
  "g5-max2-thr1000|-unroll-max-count=2 -unroll-threshold=1000"
  "g5-max2-runtime-off|-unroll-max-count=2 -unroll-runtime=false"
)

if [ -n "$ONLY" ]; then
  SELECTED=()
  for entry in "${CONFIGS[@]}"; do
    if [[ "${entry%%|*}" =~ $ONLY ]]; then SELECTED+=("$entry"); fi
  done
  [ "${#SELECTED[@]}" -gt 0 ] || { echo "ONLY=$ONLY matched no configuration" >&2; exit 1; }
  CONFIGS=("${SELECTED[@]}")
fi

# The decision column is target independent: vec_decisions (see
# target_common.sh) keeps the normalized remark lines that ARE a vectorizer
# decision --- "vectorized loop (vectorization width: N, interleaved count:
# M)", "loop not vectorized: <reason>", and the two reduction verdicts --- and
# the column says whether that set changed. The toy's hand-written table of
# four DebugLocs does not generalize to a target with hundreds of loops, and
# the objdump attribution (scripts/remark_attribution.py) is what settles
# which function a changed decision belongs to.

mkdir -p "$OUT/bin" "$OUT/strip" "$OUT/remarks" "$REMARK_DIR"

# ---------------------------------------------------------------------------
# build phase
# ---------------------------------------------------------------------------
build_one() {           # build_one <name> <knob args...>
  local name="$1"; shift
  local log="$REMARK_DIR/$name-build.log"
  local td="$SWEEP_TD/$name"
  local t0 t1 rc=0
  t0=$(date +%s%N)
  build_variant "$td" "$log" "$@" || rc=$?
  t1=$(date +%s%N)
  printf '%-30s %5d ms  rc=%d\n' "$name" $(( (t1-t0)/1000000 )) "$rc"
  if [ "$rc" != 0 ]; then
    echo "$name" >> "$OUT/build-failures.txt"
    tail -5 "$log" | sed 's/^/      /'
    return 0
  fi
  cp "$td/$TRIPLE/release/$BIN_NAME" "$OUT/bin/$name"
  cp "$OUT/bin/$name" "$OUT/strip/$name"
  strip -s "$OUT/strip/$name"
  remark_set "$log" "$OUT/remarks/$name.raw"
  remark_set_normalized "$log" "$OUT/remarks/$name.norm"
  # Correctness: the checksums must match the baseline's (SPEC.ja.md 10).
  run_correctness "$OUT/bin/$name" "$OUT/remarks/$name.checksums"
  text_hash "$OUT/bin/$name" > "$OUT/remarks/$name.texthash"
  rm -rf "$td"
}

if [ "$BUILD" = 1 ]; then
  rm -rf "$OUT" "$SWEEP_TD"
  mkdir -p "$OUT/bin" "$OUT/strip" "$OUT/remarks"
  echo "== baseline =="
  build_one baseline
  echo "== sweep: ${#CONFIGS[@]} configurations =="
  for entry in "${CONFIGS[@]}"; do
    name="${entry%%|*}"
    args="${entry#*|}"
    # shellcheck disable=SC2086
    build_one "$name" $args
  done
fi

# ---------------------------------------------------------------------------
# analysis: checksums, .text hashes, remark diffs
# ---------------------------------------------------------------------------
BASE_SUM="$OUT/remarks/baseline.checksums"
BASE_TEXT="$(cat "$OUT/remarks/baseline.texthash")"
SUMMARY="$OUT/summary.tsv"
printf 'config\tknobs\tchecksums\ttext_vs_base\traw_added\traw_removed\tnorm_added\tnorm_removed\tdecision_change\n' > "$SUMMARY"

vec_decisions "$OUT/remarks/baseline.norm" "$OUT/remarks/baseline.decisions"
echo "baseline vectorizer decisions: $(wc -l < "$OUT/remarks/baseline.decisions") lines"

TIMING_SET=(baseline)
for entry in "${CONFIGS[@]}"; do
  name="${entry%%|*}"; knobs="${entry#*|}"
  if [ ! -f "$OUT/remarks/$name.norm" ]; then
    printf '%s\t%s\tBUILD-FAILED\t-\t-\t-\t-\t-\t-\n' "$name" "$knobs" >> "$SUMMARY"
    continue
  fi
  if diff -q "$BASE_SUM" "$OUT/remarks/$name.checksums" >/dev/null; then sums=MATCH; else sums=MISMATCH; fi
  if [ "$(cat "$OUT/remarks/$name.texthash")" = "$BASE_TEXT" ]; then th=same; else th=differ; fi
  ra=$(comm -13 "$OUT/remarks/baseline.raw" "$OUT/remarks/$name.raw" | wc -l)
  rr=$(comm -23 "$OUT/remarks/baseline.raw" "$OUT/remarks/$name.raw" | wc -l)
  na=$(comm -13 "$OUT/remarks/baseline.norm" "$OUT/remarks/$name.norm" | wc -l)
  nr=$(comm -23 "$OUT/remarks/baseline.norm" "$OUT/remarks/$name.norm" | wc -l)
  vec_decisions "$OUT/remarks/$name.norm" "$OUT/remarks/$name.decisions"
  if diff -q "$OUT/remarks/baseline.decisions" "$OUT/remarks/$name.decisions" >/dev/null; then dc=no; else dc=YES; fi
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$name" "$knobs" "$sums" "$th" "$ra" "$rr" "$na" "$nr" "$dc" >> "$SUMMARY"
  # Timing set: group 0 always (it is the in-sweep noise probe), plus every
  # configuration whose .text changed. A configuration with an identical
  # .text is the same machine code and cannot differ in time, however many
  # remark lines moved (results.md Day 0 section 17).
  if [ "${name#g0-}" != "$name" ] || [ "$th" = differ ]; then
    TIMING_SET+=("$name")
  fi
done

column -t -s$'\t' "$SUMMARY" | tee "$OUT/summary.txt"
echo
echo
echo "timing set (${#TIMING_SET[@]}): ${TIMING_SET[*]}"
printf '%s\n' "${TIMING_SET[@]}" > "$OUT/timing-set.txt"

# ---------------------------------------------------------------------------
# timing phase
# ---------------------------------------------------------------------------
[ "$TIME" = 1 ] || exit 0

if [ -z "${MDE:-}" ] && [ -f "$AA_STATS" ]; then
  MDE=$(python3 -c "import json,sys;print(json.load(open(sys.argv[1]))['mde'])" "$AA_STATS")
fi
MDE="${MDE:-0.03}"
echo "frozen MDE from $AA_STATS: $MDE"

LBL=()
while read -r n; do LBL+=(--label "$n=$OUT/strip/$n"); done < "$OUT/timing-set.txt"
WL=()
for w in "${WORKLOADS[@]}"; do WL+=(--workload "$w"); done

"$REPO/scripts/bench.py" run \
  --cpu "$BENCH_CPU" --warmup "$WARMUP" --runs "$RUNS" \
  "${LBL[@]}" "${WL[@]}" --out "$OUT/$BENCH_OUT_NAME.json" --progress

"$REPO/scripts/bench.py" stats "$OUT/$BENCH_OUT_NAME.json" --base baseline \
  --seed 20260921 --resamples 10000 --mde "$MDE" \
  --json "$OUT/$BENCH_OUT_NAME-stats.json" | tee "$OUT/$BENCH_OUT_NAME-stats.md"

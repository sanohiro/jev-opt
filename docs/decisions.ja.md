...
### 105. MDE は A/A の 2 倍・下限 1%(3% の方針下限を撤回)。「ノイズでなければ入れる」
- **知見**: オーナーの指示(2026-09-24): 「目的は O3 + native + fat LTO + PGO の上の +α。対象は何でもよい。小さくても再現する効果を恣意的な床で捨てるな。ノイズでなければ入れる」。これまでの MDE は `max(2 × 最悪の per-case 半幅, 3%)` をバッチごとに再計算し、3% は決定 27 の方針下限だった。v2 規則 `max(2 × 対象の登録 A/A パネルの最悪 per-case 半幅, 1%)`(対象ごとに凍結)で既存 oracle を事後に読み直した(`results.md` §171。再計測なし)。v2 MDE は zopfli 訓練 **1.838%**(§165 aa2/binary 0.919%)、holdout **1.00%**(0.39% → 下限)、hintbench **3.752%**(§119 ヌルパネル、k8 の 1.876%。§162 の固定クラス A/A なら 3.176%)、jaq A2 **3.26%**(§140 holdout の aa、objsearch 1.63%)。**下がるのは zopfli だけ**で、hintbench と jaq は 1 ケース(k8、objsearch)のせいで 3% より上がる。zopfli: 組み合わせは訓練 +1.811% / +2.063%、holdout +1.77% で、**ラウンドバッチが 1.838% に 0.027 pt 届かない**(確認と holdout は通る)。`squeeze.rs:325 unroll_count_8` は +1.256% / +1.285% で訓練は平坦、holdout +1.33% は 1.00% を超える。有害は 2 → 5(`find_longest_match` の `inline_never` −2.23 / −2.68、`inline_always` −2.01 / −2.04、`get_best_lengths` の `inline_never` −2.01 / −2.17。`lz77_optimal_run` の `inline_never` は確認 −1.829% で 0.009 pt 足りない)。hintbench: 凍結によって 4 本が変わる(k3 ループ `unroll_count_2` +4.23 / +3.94 が効果に、k1 `inline_never`・k4 と k8 ループの `interleave_count_2` が有害に)。1〜3% 帯の k5 幅 16(+2.65 / +2.85)などは v2 でも平坦で、カーネル別 MDE(1.0〜1.27%、k8 のみ 3.75%)を採った場合にだけ効果になる。jaq: 1〜3% の帯は空。`write_until inline_never`(−3.75 / −3.22)が有害から外れる。感度: zopfli の `h` を holdout パネル(0.391%)か訓練パネルの集計 A/A(0.414%)にすると MDE 1.00% で、組み合わせも 325 も 3 脚とも効果になる。oracle 走行内の A/A(0.83%)なら 1.66% で組み合わせだけ効果。driver は凍結 `--mde` を 0.03 で床打ちし(`jev_search.py` 3885・4983 行)、`bench.py` も `max(2 * worst, 0.03)`(350 行)なので、今のままでは v2 を走らせても 3% になる。
- **判断**: (a) 新しい事前登録はすべて MDE v2 を使う。`h` は対象の登録 A/A パネル(同一バイナリの脚だけ)の最悪 per-case 半幅で、計測前に対象ごとに凍結し、バッチごとには再計算しない。効果 = ラウンドバッチと確認バッチの両方が同じ側で MDE を超えること(決定 80(a) と同じ論理、閾値だけ変える)。記事ではさらに、測った場合は holdout の一致を添える。(b) 既存の結果は元の規則(v1、3%)のまま残し、§171 の事後表を脚注として付ける。後から「効果」と呼び直さない(決定 100(a) は維持)。(c) zopfli の +1.8% の組み合わせは「事後の読み直しで、確認バッチと holdout では v2 MDE を超えたが、ラウンドバッチは 0.027 pt 足りない境界例」と報告する。v2 の文言どおりでは確認済みの α ではない。zopfli の `h` をどのパネルで登録するか(訓練の per-case 0.919% / holdout 0.391% / 集計)は**オーナー判断**とし、エージェントは結果を見て選び直さない。(d) 今後の実プログラム oracle は protocol v2(決定 104)と MDE v2 の両方で走らせる。hintbench を protocol v2 で走らせるときは `--mde` を A/A 由来の値(§162 の 3.18%)に凍結し、n=8 で MDE が膨らまないようにする(決定 104(b))。(e) v2 の最初の計測の前に、driver の床(`jev_search.py`、`bench.py`)を 0.01 にして凍結値を渡せるようにする(コード変更、未実施)。(f) 1〜2% の主張には決定 94 の標本変動の注意がより強く効くので、Jev vs ランダムで 1〜2% を言うときは n≥3(決定 96)を必須とする。
- **影響**: spec §2 の採用規則と MDE の文(「ノイズと最小効果量」)、§7 の MDE 参照。`jev-opt.toml` の `mde` キー(注記の「floor 0.03」、既定値)と driver の床は次の driver 変更で(未実施)。HANDOFF §2 の行 16、§3.4、§4 の行 7。`results.md` §171。

### 106. MDE v2 の確定: 集計 A/A の半幅 × 2、下限 1%(ケース別主張はケース別 A/A)。driver の 3% 下限を撤去
- **知見**: §171 の事後の読み直しでは、`h` を「登録 A/A パネルの最悪 per-case 半幅」と読むと zopfli の訓練 MDE が 1.838% になり、組み合わせ(+1.811% / +2.063%、holdout +1.77%)はラウンドバッチだけが 0.027 pt 足りず、確認バッチと holdout は超えていた。見出しは集計(幾何平均)比なので、その MDE は集計の A/A から取るのが筋で、per-case の最悪値は集計の主張に対して 2〜3 倍保守的だった。オーナー判断(2026-09-24「おすすめでなおして」)。登録パネルの集計 A/A 半幅は zopfli 訓練 0.414%(§165 aa2)、holdout 0.187%、hintbench §162 0.317%(§119 ヌルパネル 0.268%)、jaq §140 holdout の aa 0.564%(`results.md` §172.1)。driver は凍結 `--mde` を 0.03 で床打ちし(`jev_search.py`)、`bench.py` も `max(2 * worst, 0.03)` だった。
- **判断**: (a) **MDE v2 確定** = `max(2 × 対象の登録 A/A パネル(同一バイナリの脚)の集計 95% 半幅, 1%)`、計測前に対象ごとに凍結。**ケース別の主張**(hintbench のカーネル別読み出しなど)はそのケースの A/A 半幅 × 2(下限 1%)。効果 = ラウンドバッチと確認バッチが同じ側で MDE を超えること(決定 80(a))。決定 27 の 3% の方針下限は撤回。確定値: zopfli 訓練 1.00% / holdout 1.00%、hintbench 集計 1.00%(カーネル別 1.00〜3.18%、§162)、jaq 1.13%(バッチ内半幅が過信気味なのでノイズの下限)。(b) zopfli の組み合わせ **+1.8%**(+1.81 / +2.06 / holdout +1.77%)と `squeeze.rs:325 unroll_count_8` **+1.3%**(+1.26 / +1.29 / holdout +1.33%)は、3 脚とも 1.00% を超える**再現性のある α** として報告する。ただし oracle は v1(3%)で事前登録したので「事後」と明記し、§168.7 の事前登録の判定(「良い」0/67)は残す。(c) 新しい走行は protocol v2(決定 104)+ MDE v2 確定で行う。driver の既定は `[evaluation] mde_floor = 0.01`、`mde_from = "aggregate"`(`bench.py stats` も同じ既定)。各走行の `rounds.jsonl`・`holdout.json`・manifest に `mde_rule`("v2-aggregate")/ `mde_floor` / `mde_from` を記録し、旧走行との比較を明示する。旧規則は `--mde-from worst_case --mde-floor 0.03` で再現する(記録済みの `mde` とビット一致、§172.4)。(d) hintbench は §162 から凍結する: `--mde 0.01 --mde-case k1=0.0158,k2=0.0135,k3=0.0224,k4=0.0163,k5=0.0170,k6=0.0100,k7=0.0253,k8=0.0318`(決定 104(b) の単一 `--mde 0.0318` を置き換える)。(e) state 文の `{mde}` は凍結 `mde` があればその値、無ければ下限(「1%」)。テンプレートは不変だが、Jev に見せる数字は旧走行(「3%」)と変わる。(f) hintbench の Jev 見出し(Exp4〜6)と jaq・zopfli の Jev vs ランダムは再採点しない。1〜2% の主張には n≥3(決定 96)を必須とする(決定 105(f) を維持)。
- **影響**: spec §2(「ノイズと最小効果量」の MDE の文、主指標の zopfli)、§7、§9 の項目 10、ヘッダ(66〜106)。`jev-opt.toml [evaluation]`、`scripts/jev_search.py`、`scripts/bench.py`、`docs/search-driver.md`。HANDOFF §2 行 16、§3.4、§4 行 7、§6。`results.md` §172。

### 107. Prompt study 2: 【聞き方ではループの正解に届かない。偽陽性を減らす聞き方は、同じだけ真陽性(k2、write_until)も消す】
- **知見**: API のみ(ビルド・計測なし)で、driver が今送っている v6 の一発要求(B0)と、site に一律な 14 通りの聞き方(ループ候補文の書き直し「v7 候補」、判定ブロックを生の事実行に、レーン数の結論行を外す、「ループが見つからない → 付けても効かない」という推論を中立化、unroller の結果と unroll 算術、remark を質問に転記、Score、2 段階、逆問い「最も遅くするのは」、1 site 1 要求、候補順の固定乱順、「ここでは何も測っていない」の 1 行、ソース抜粋を外す)を各 3 回送り、oracle(MDE v2)で採点した。**ループの正解(hintbench k3 `unroll.count=4`、k5 / k8 `vectorize.width=16`、zopfli `squeeze.rs:325` の `unroll.count=8`)は、どの変種でも一度も argmax にならなかった**(3/3 反復とも)。P の最大は k3 の 0.19(乱順、B0 の範囲と接していて分離できない)、他の 3 つは 0.05 以下。事前登録の「動いた」は v7 候補文(L1)の k5 0.00 → 0.01、k8 0.00 → 0.03、325 0.00 → 0.05 だけで、選択は変わらない。state 中の 2 つの疑わしい記述(k5 の「1 レジスタに収まる最大幅は 8」、325 の「付けても効かない」は oracle と矛盾)を外しても P は動かなかった。逆問いでは Jev は正解の腕を「最も有害」とみなし(k5 0.28、k8 0.26、325 0.52)、Score では「変化なし〜遅くなる」に置く。つまり言い回しの問題ではなく、Jev はこれらのヒントが悪いと予想している。関数側は頑健で、B0 は v6 でも決定 87 を再現(hintbench 7/8、k2 を当て k6 が唯一の外れ、jaq 13/15 で `write_until inline(always)` を当てる)。v6 の B0 ではループの有害 argmax は既に 0(hintbench 0/4、zopfli 0/2。決定 87 の k4 幅 16 は post_vectorize の事実で消えている)。第 2 段(偽陽性を減らす): 害の条件を候補文に足す(H1)、事実行(H2)、KEEP 優先の問い(H3)、逆問いによる拒否(H4)、2 段階(H5)は、有害な腕への確率質量を 2〜10 倍減らす(zopfli 関数 0.23 → 0.06〜0.14、hintbench ループ 0.12 → 0.01〜0.02)が、**5 つすべてが hintbench k2 の `inline(always)`(+67.8%)を 3/3 で KEEP に変え**、H4 以外は jaq の `write_until` も失う。事前登録どおり C1(最良の組み合わせ)は送らなかった。465 要求、喪失 4(いずれも分割の前の大きな Score 要求)、費用 $0。途中で 3 つの逸脱を登録した(Score 要求の分割、`--resume`、jaq の site 分割。いずれも送る前に `results.md` §173.7 に記録)。`results.md` §173〜§175、`docs/experiments/jev-prompt-study-2/`。
- **判断**: (a) ループの正解は、一律な聞き方の変更では Jev に選ばせられない。「聞き方で近づける」(決定 69・73 の路線)はループについては打ち切る。ループの正解に届いた実績は、測って結果を返す探索(決定 92 の k3)だけで、今後もそこに頼る。(b) v7(ループ候補文)は凍結しない。効果が P 0.01〜0.05 で、選択を変えない。(c) 偽陽性を減らす一律の変更は、同じ state を持つ真陽性も一緒に消す(k2 と k6 は state の上で区別できない)。偽陽性を止めるのは今後も速度ゲート(決定 101)で、聞き方ではない。(d) 325 の post_vectorize 行の「付けても効かない」という推論は oracle と矛盾するので、次に state を改訂するときに中立の文(L4)に直す提案として残す(Jev の答えは変わらなかったので優先度は低い)。
- **影響**: HANDOFF §3.4・§4(v7 は提案せず、新行 10 は「済・不採用」)。spec §6(聞き方の節)に「ループは探索で、聞き方では届かない」を次回反映。`scripts/jev_prompt_study2.py`。

### 108. マーク選定を Jev に任せられるか: 【条件付きで可。Claude が見たのと同じバイナリの事実を渡せば事前登録の基準は通るが、oracle の正解の被覆は機械規則と同点で、上回らない】
- **知見**: API のみ(ビルド・計測なし)で、perf 表の関数を一律・アルファベット順に並べ(reach 1% 以上、C を除く。jaq 124 行、zopfli 26 行、hintbench 8 行)、関数ごとに Choice(mark / skip、Q1)と Score(Q2)を各 3 回送った。入力は 2 通り: v1 = self / reach(訓練・holdout)・own symbol・サイズだけ、v2(176.7 で追加登録)= それに crate・ケース別 reach・LTO 後の命令数・後方分岐数(ループ)・ホスト数(Claude が jaq のマークで使った事実と同じ種類)。さらに混成 Q3(reach ≥ 5% かつループありは機械的に mark、reach < 1% とサンク(≤ 8 命令)は skip、残りの灰色帯だけ Jev に Q1)。結果(中央値): jaq の Claude との一致 (a) は v1 Q1 0.53・Q2 0.53、**v2 Q1 0.67**・Q2 0.47・**Q3 0.67**。zopfli は v1/v2 Q1 と Q3 が 1.00、Q2 は 0.67。oracle の正解の被覆 (b) は、jaq ではどの読み出しも 1/2 で、対照の top-N reach(`write_until` を取る)・top-N self(`Val::hash` を取る)と同点。Jev は `write_until`(reach 順位 10、P 0.78〜0.83)は毎回取り、`Val::hash`(reach 順位 83、self 1.2%、ループ 90)は取らない(v1 P 0.30〜0.38、v2 0.45〜0.46、Q3 で 1 回だけ 0.52)。つまり Jev は「own symbol とループに重みを付けた reach 順位」のように振る舞い、reach で見えないものは拾わない。zopfli は正解 2 つが reach 1・2 位なので (b) は何も分けない。単一の機械的順位で両対象とも (a) ≥ 0.6 に届くものは無い(top-N self 0.80 / 0.33、top-N reach 0.40 / 0.83)が、v2 Q1 は 0.67 / 1.00(ただし jaq で 25 行と多めに付けた分を含む。N に揃えると 0.60 / 0.67 で基準ちょうど)。混成 Q3 は質問数 81%(jaq)・54%(zopfli)で v2 Q1 と同等、hintbench では規則だけだと落とす k2(ループ無し、+67.8% の正解)を Jev が拾う(3/3 対 2/3)。hintbench の Q1 は k1・k7(ループ無しでインライン化)を skip、正解 k2・k3・k8 は毎回 mark。79 要求、503 は 1223 回(ほぼ全部 60 KB 近い jaq 要求)、喪失 0、請求 $0(定価 $0.048)。`results.md` §176〜§177、`docs/experiments/jev-marks-study/`。
- **判断**: (a) 事前登録の判定規則(176.6)どおり「Jev はマークを選べる」とする。ただし条件付き: v1(perf の割合だけ)では通らず、v2(命令数・ループ数・ケース別 reach などのバイナリの事実)を渡したときだけ通る。(b) 通り方は「機械規則と同点」であって「上回る」ではない。Claude の jaq マークが両方の正解を含んでいたのは、reach 順位 83 の `Val::hash` を self の表から拾ったからで、Jev はそれを再現しない。記事ではこの区別を書く。(c) `--marks-by jev` を driver の選択肢として**設計のみ提案**する(実装しない): 入力は `scripts/jev_marks_study.py --inputs v2` の表(perf の `perf_hotness.py --inline` と `inline_structure.py` から機械的に作る)、形は混成 Q3(明確な行は規則、灰色帯だけ Jev の Choice、3 回送って中央値 P > 0.5)、出力は `jev-marks.txt`。既定は現行どおり人・Claude のマーク。(d) Score の聞き方(Q2)はマーク選定でも Choice より悪い(study 2 と同じ)ので使わない。(e) ソース抜粋を足す Q4(探索的)は未実施。
- **影響**: HANDOFF §3.4・§4(行 11)。driver は変更しない。spec §1(1)(マーク)に「Jev でも選べる(条件付き)、ただし oracle の正解の被覆は機械規則と同点」を次回反映。`scripts/jev_marks_study.py`。
- **追記(phase 3、2026-09-25、`results.md` §176.8・§178)**: 決定 73 の教訓(判定行を質問の横に置く、選択肢文の長さを揃える)をマーク選定に当てた。M1 = 関数ごとの機械的判定行(LTO 後のループの有無と後方分岐数、呼ばれるかインライン化か、own symbol、他 crate の汎用関数がこのプログラムの LTO 単位で実体化されたか、ケース別の割合、reach 順位、self 順位)、M2 = ケースごとに mark / skip を聞いて和集合、M3 = 一律の適用条件文(「関数属性は呼ばれるか定数引数でインライン化される場合のみ、ループ metadata は LTO 後にループが残る場合のみ速度を変えうる」)を選択肢に置く、M4 = M1 + M3、M5 = M1 + M2 + M3。各 3 回、186 要求、喪失 0、$0。**事前登録の「救済」はどれも満たさない**: `Val::hash` は M1 で P 0.45 → 0.79、M3 で 0.87、M4・M5 で 1.00 と入るが、同時に jaq のマーク数が 25 → 47 / 65 / 109(124 中)に膨らみ、どの参照集合にも無い関数が 10 → 16 / 32 / 70(基準 ≤ 10)。M2 は一致 0.80 に上がり書き込み経路の 2 本を拾うが `Val::hash` は拾わず、はみ出し 23。zopfli はどの変種でも一致 1.00・正解 1/1、hintbench は M3 で k2(ループ無し)を落とす。判定行は決定 73 と同じく「従われる」が、順位ではなく閾値として従われる(条件を満たす行を全部 mark する)。`Val::hash` を近傍から分ける機械的な 1 行は無く、Claude の選択は object 経路についての判断だった。**判断**: 決定 108 は変えない(`--marks-by jev` は v2 の Q1 / 混成 Q3 の文面のまま設計のみ提案)。判定行・適用条件文・ケース別の問いはマーク選定には採用しない。記事では「Jev は事実に従うが、事実で分けられないものは分けない」の実例として書ける。

### 109. マーク選定を「規則の種(reach 上位 ∪ self 上位)+ 灰色帯 Jev」に固定。手動の点を無くす
- **知見**: 決定 108 の marks study(API のみ)で、単一の機械的順位はどれも両対象を覆えなかった(jaq の正解 `write_until` は reach 順位 10、`Val::hash` は reach 順位 83 だが self 順位 13)一方、top-N reach ∪ top-N self の和集合なら両方を含む。Jev は reach で見える側しか拾わず、判定行・適用条件文は閾値として従われてマークを膨らませた(§178)。オーナーの決定(2026-09-25): 種は規則で、残りの灰色帯だけ Jev に聞く。実装 `scripts/target_marks.py` で API 検証(`results.md` §179): jaq は種 26 行 + 灰色帯 85 問中 8 = **34 行**、Claude の凍結マーク 15/15 と正解 **2/2(どちらも種)**。zopfli は種 10 + 灰色帯 14 問中 4 = **14 行**、凍結 6/6(`find_longest_match` は灰色帯で P 0.87〜0.91)、`lz77_optimal` 被覆。hintbench は 8 行が同順位なので種 8・灰色帯 0・要求 0、k2/k3/k8 被覆(行の一覧が凍結マークそのものなので選定の検証にはならない)。coordinator の期待「hintbench の k2 を灰色帯で Jev が救う」は §176.7 の混成規則(reach ≥ 5% かつループあり)の話で、採用規則には当てはまらないと事前登録で訂正した。9 要求、503 は 196 回、喪失 0、請求 $0(定価 $0.0064)。最終集合は凍結の約 2.3 倍(種だけで 1.7 倍: reach 上位と self 上位の重なりが小さい)。
- **判断**: (a) **規則**: 一覧 = perf inline 表のうち訓練か holdout で reach ≥ 1% または self ≥ 1% の行(C を除く。study の入力 v2 と同じ表・id・state)。除外(マークしない・問わない)= IR 無し / C、サンク(LTO 後の命令数 ≤ 8、study 176.7 の閾値。オーナーの「1 命令のサンク」を字義どおり 1 にするとこの表では何も除けない)、1% 未満。**種**(問わずにマーク)= 訓練 reach の上位 N ∪ 訓練 self の上位 N、除外でもコンパイラ生成(`drop_glue` / `drop_in_place` / shim)でもない行の中で順位付け、同順位はアルファベット順。「own IR symbol」は「plugin が IR を持つ Rust 関数」と読み、最終バイナリにシンボルが残るか(study の `own symbol` 列)は条件にしない(凍結マークの `seq`・`str_fold`・`get_best_lengths`、hintbench の 6 カーネルはシンボルが無いが plugin は解決し oracle も測った)。**灰色帯** = 残りで種の行にまだ一致しない行。study の Q1 文面(判定行なし、適用条件文なし)で Choice {mark, skip}、3 回、中央値 P ≥ 0.5 でマーク。(b) **N** = 凍結マークの本数(jaq 15、zopfli 6、hintbench 8)、`--marks-n` で変更可、凍結マークの無い新対象は 8(ヘッダに記録)。最終集合が N を超えてよく、本数を記録する。(c) **凍結マークは既存の比較のために残す**。`target_marks.py` は `jev-marks.txt` を書かず `jev-marks.jev.txt` と `jev-marks.jev.rationale.md` を書く。凍結ファイルを置き換えるかは対象ごとにオーナーが新しい事前登録で決める。新しい対象は `target_marks.py`(`target_sites.sh marks`、既定は dry run、`MARKS_JEV=1` で問い合わせ)を使う。(d) **driver の既定マーク**: `jev_search.py --marks` は任意。明示の `--marks` → 対象の最新の生成済み `jev-marks.jev[.vN].txt` → 無ければ `target_marks.py` で 1 回だけ生成。生成物(マーク・根拠・要求 JSONL)はその時点で凍結入力となり、driver は黙って再生成しない。`--marks-regenerate` は次の版番号のファイルに書く(古い比較は古いファイルのまま)。`run-manifest.json` に使ったファイル・sha256・由来(explicit / existing generated / generated now と Jev ログ)を記録する。凍結対象の記録済みコマンドは明示の `--marks` のまま。(e) **perf の表が無ければ準備として自動で取る**: 他の計測・ビルド(`bench.py` / `jev_search` / `cargo`)が走っていれば拒否、perf が無ければ「`scripts/perf_local.sh setup` を先に」で止める(唯一のエラー)、plugin off の基準バイナリで訓練(探索)ケースだけ `SETS=training perf_marks_profile.sh` → `perf_hotness.py --inline` → `inline_structure.py`、各段のコマンドと所要時間を表示。この経路は実装・スタブ付き単体テスト(15 件)済みで、**この回には一度も実行していない**(3 対象とも表がある)。
- **影響**: `scripts/target_marks.py`(新規)、`scripts/test_target_marks.py`、`scripts/target_sites.sh`(`marks` サブコマンド、`MARKS` を環境変数で上書き可)、`scripts/jev_search.py`(`--marks` 任意、`--marks-regenerate`、`--marks-n`、manifest の `marks_provenance`)、`docs/search-driver.md`「Marks: mechanical seed ∪ Jev gray zone (decision 109)」、`README.md`(新規)、spec §1(1)・§8 `mark`・§10、AGENTS.md の準備の項、HANDOFF §2 行 4 注・§4 行 11 → 済。`results.md` §179。

### 110. state からソース抜粋を外す(`--source-excerpt none`)を新規走行の既定候補に: 要求サイズ【phase A 65.3 → 46.3 KB(hintbench)、54.6 → 38.6 KB(zopfli)、本文全体で −17〜29%】、着弾率【悪い日に 33/60 → 39/60、有意差なし】、関数属性の成績は維持(§174 L13)。ただしループで k4 が有害側に倒れるため既定にはしない(opt-in)
- **知見**: 503 は提供側(Vercel / TypeSafe)のその日の状態で、要求サイズに依存する(`docs/search-driver.md`「Gateway and retries」の 2026-09-23 の probe、HANDOFF §6)。Vercel Pro に上げても提供側の 503 は直らないので、効くのは要求を小さくすることだけで、証拠のある手段は prompt study 2 の L13(ソース抜粋を落とす)だけだった。driver に `--source-excerpt {full,none}` を入れた(既定 `full`)。`none` は各抜粋を 1 行「(source excerpt omitted from this request)」に置き換え、判定ブロック・remarks・post_vectorize・インライン結果・platform ブロックなど機械的な事実の行は全部残す。state 形式は別名(`state-v5.0-noexcerpt-2026-09-25`、`state-v5.1-…`、`state-v6.0-…`)で、凍結テンプレートは不変。study 2 の round-1 要求を driver の `none` で作り直すと、L13 で実際に送った本文と **1 行(state 形式の文字列)だけ**違い、質問は同一(hintbench A/B、zopfli A/B)。したがって §174 の L13 の数字はこの描画の数字。サイズは state で −41〜53%、HTTP 本文で −17〜29%(質問側に判定ブロックがあり、そちらが本文の大半になる)。着弾 probe(事前登録 §180、各本文 10 回、1 回 1 試行、3 s 間隔、full/none 交互、逐次): 悪い日(120 送信中 503 が 48)で full 33/60、none 39/60。本文別では none が 4 勝 1 分 1 敗(hintbench A だけ 7 → 5)。Fisher 両側 p = 0.35 で偶然の範囲。サイズ別の着弾は < 30 KB 80%、30〜50 KB 56%、> 50 KB 40% で、サイズ依存は再現した。Choice の再確認(driver の `none`、各 3 回、全着弾): hintbench 関数 7/8(k2 は 3/3)、zopfli 関数 3/6、zopfli ループ 4/5 と有害 0 は §174 の L13 と中央値一致。**hintbench のループでは k4 が oracle で有害な `vectorize_width_16` に 3/3 回倒れる**(§174 の L13 は 2/3、B0 は 0/3 で KEEP)。これは事前登録の時点で §174 の表から「(b2) は hintbench で成り立たない」と明記していた。依頼文の「ループは不変」は zopfli だけで正しい。`results.md` §180・§180.1。
- **判断**: 事前登録の読み「(a) 成立・(b1) 成立・(b2) 不成立」に従い、**`none` は新規走行の既定にしない**。opt-in の選択肢として残す。新規走行も凍結比較も `full` のまま。理由はループ(k4)の後退で、ゲートウェイの結果ではない(着弾は none が上だが有意でない)。503 への現行の対処は変えない: 試行回数と壁時計の上限(決定 92・95)と、**逐次送信**(並行送信を避ければ 429 は出ない)。Vercel Pro は解決策ではない(503 は提供側で、サイズとその日の状態に依存する)。**提案のみ**: phase A だけ抜粋を外す変種(関数は L13 の成績、ループは B0 の描画、最大の本文である phase A が −29%)。未測定の新変種なので、採るなら新しい事前登録で study 2 と同じ採点をしてから。
- **影響**: `scripts/jev_search.py`(`--source-excerpt`、`NoExcerptSource`、`SOURCE_OMITTED`、`STATE_FORMATS` の noexcerpt 3 項目、manifest・`rounds.jsonl`・JSONL の `source_excerpt`)、`scripts/jev_noexcerpt_probe.py`(新規)、`docs/search-driver.md`「Source excerpts dropped」、HANDOFF §6 のゲートウェイの項。ログ `docs/experiments/jev-prompt-study-2/noexcerpt/`。

### 111. Jev の 503 は provider の過負荷(文書上は 529)がゲートウェイを素通りしたもの。直接 API を選べるようにする(既定は従来どおりゲートウェイ)
- **知見**: docs.typesafe.ai(2026-09-25 時点、WebFetch で確認。手元に TypeSafe の鍵が無く、実 API には未検証)によれば、TypeSafe 直接 API のベースは `https://api.typesafe.ai`、パスは `/v1/systemone`(ゲートウェイと同じ)、鍵は `Authorization: Bearer <TYPESAFE_API_KEY>`。文書化されたエラーは 401(鍵欠落・不正)、422(バリデーション失敗)、429(レート超過。250k tokens/s、1,200 req/min)、**529「TypeSafe is temporarily overloaded」(リトライ対象)**。**503 は TypeSafe の文書化されたコードではない**。body は `state`・`questions` に加え **`model` が必須**(`"model" type="string" required`、例は `"jev-latest"`)。SDK 既定モデルも `jev-latest`(docs.typesafe.ai/sdk/python/usage の `TYPESAFE_DEFAULT_MODEL`)で、ゲートウェイの名前空間付き id `typesafe-ai/jev` とは別物。429/529 とも `Retry-After` を送ると文書に明記されてはいない。Vercel AI Gateway(`https://ai-gateway.vercel.sh/typesafe/v1/systemone`、`model: typesafe-ai/jev`)は provider のエラーをそのまま通し、1 回だけ試行する。価格は直接 API が入力 $0.042/1M・出力無料・**無料枠なし**。TypeSafe のステータスページには 2026-09-23 20:56 UTC と 2026-09-24 07:18 UTC 解消の「elevated latency / connection issues」の 2 件があり、これは decision 92 (d) の 2026-09-23 悪い日のプローブ(phase A 85 KB 0/6 など)の時間帯と重なる。手元の 503 頻度は、探索走行では 1 走行あたり 19〜120 回(壁時計の 1〜8%、各走行の `run-manifest.json` の `gateway` ブロック参照)、API のみの study では数千回(決定 108 は 1223 回、決定 109 は 196 回)で、後者は大きな要求を連続で・対象 3 つ並行で送ったことによる。つまり手元の 503 も、頻度の差も、provider 側の過負荷(文書上の 529)がその日の状態・要求サイズ・並行度に依存して起きている、という既存の理解(decision 92 d)と符合する。
- **判断**: (a) `[jev] endpoint`(既定 `"auto"`)/ 環境変数 `JEV_ENDPOINT` / CLI `--jev-endpoint` で `gateway`(既定と同じ動作)か `direct` を選べるようにした。優先順位は **CLI > `JEV_ENDPOINT`(環境または `.env`)> `jev-opt.toml [jev] endpoint` > auto**。`auto` は「どちらの鍵が存在するか」で決める: `TYPESAFE_API_KEY` のみ → `direct`、`AI_GATEWAY_API_KEY` のみ → `gateway`、両方 → `gateway`(既定を維持)、どちらも無ければ両方の変数名を挙げてそこで初めて `sys.exit`(`--dry-run`/`--print-state` はクライアントを作るだけで鍵を要らないので、構築時には決して例外を投げない。§`resolve_endpoint`)。今回 `jev-opt.toml` の `endpoint` を `"/v1/systemone"`(URL パス)から `"auto"`(ルート選択)に意味を変えたので、パスは新しいキー `path` に移した。(b) 529 は既存の 5xx リトライ規則にそのまま含まれる(`status >= 500` で判定していたので判定自体は変更不要)。追加したのは **`Retry-After` を 429 だけでなく 529 を含む全てのリトライ対象ステータスで読む**こと(文書は 429/529 いずれにも保証していないため)。ログは attempt ごとに `endpoint`(gateway/direct)・`http_status`・`retry_after` を記録済み(decision 92 d の `attempt_log` を拡張)。(c) `direct` は **文書だけを根拠に実装し、このプロジェクトでは実 API に対して一度も検証していない**(TypeSafe の鍵が無いため)。オーナーが鍵を用意でき次第、同一 body を両ルートに各 10 回送る A/B probe(decision 92 d の 2026-09-23 probe と同型)を事前登録の次の一手とする。(d) Vercel Pro へのアップグレードは対策にならない(503/529 は provider 側の状態で、Gateway の課金プランとは無関係。decision 92 d・110 のまま)。(e) 送信は今までどおり**逐次**(並行させると 429 が出る。decision 92 d)。(f) 実 API 未検証のまま既定を変えるのは危険なので、**既定は従来どおり `gateway`**(今日的挙動と同一、`auto` が単一の鍵から `gateway` を選ぶことで再現される)。(g) `--jev-smoke`(単発 Choice、`--target`/`--proposer`/`--out` 不要、ビルドも計測も無し)を追加し、鍵を持つ利用者が `direct` を信用する前に 1 回で確認できるようにした。クライアント自身が選ばれたルート・その理由・HTTP ステータス・応答のトップレベルキーを表示する(鍵は表示しない)。
- **影響**: `jev-opt.toml`([jev] の `endpoint`/`path`/`direct_base_url`/`direct_key_env`/`direct_model`。**このファイルの sha256 が変わる**ので、以後の run-manifest の `config_sha256` はこの変更をまたぐ比較には使えない)。`.env.example`(`TYPESAFE_API_KEY`、`JEV_ENDPOINT`)。`scripts/jev_search.py`(`resolve_endpoint`、`dotenv_get`、`JevClient.__init__`/`ask`(`endpoint`/`endpoint_source`/`key_env`/`base_url`、smoke 出力)、manifest の `jev_endpoint`/`jev_endpoint_source`/`jev_base_url`/`jev_key_env`、CLI `--jev-endpoint`/`--jev-smoke`、`jev_smoke()`)。`scripts/test_jev_client.py`(新規、スタブ HTTP: gateway/direct の URL・モデル・鍵環境変数、優先順位、auto 選択の 4 通り、529 のリトライと `Retry-After`、鍵がログに出ないこと)。`docs/search-driver.md`「Gateway and retries」。`README.md`「Prerequisites」。HANDOFF §6 のゲートウェイの項(503/529・auto 選択・`--jev-smoke` を追記)。`jev_prompt_study2.py`/`jev_noexcerpt_probe.py` は `JevClient` を呼ぶだけでエンドポイントを複製していないため無変更。SPEC.ja.md §8 の `endpoint = "/v1/systemone"` 行(旧キーの意味のまま)は次回の spec 反映で `path` に直す(107・108 と同じ扱い、今回は編集範囲外)。

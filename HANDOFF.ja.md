...
この文書は、次に作業する人・エージェント(Claude でも Codex でも)が、経緯と現状と予定と作法を全部引き継げるように書いたもの。規約の短縮版は `AGENTS.md`(英語)。仕様は `SPEC.ja.md`、知見と判断の履歴は `docs/decisions.ja.md`(109 項目、番号で参照)、数値は `results.md`(章番号で参照、§179 まで)。利用者向けの入口は `README.md`(英語)。この 3 つが正本で、本文書はそこへの案内図。
...
| 4 | マークは「Jev の判断対象を単純にする」ための準備。人が直接指示してもよい。Claude が「不要」と切ったのは読み違え。**既定の手順は決定 109**(`scripts/target_marks.py`: 種 = reach 上位 N ∪ self 上位 N、灰色帯だけ Jev。凍結マークは既存の比較用に残す) | 決定 57・58・109 |
...
| 10 | Prompt study 2(API のみ): 聞き方でループの正解に届くか、偽陽性を減らせるか | 一律な聞き方の変更で k3 / k8 / 325 を拾えるなら v7 として凍結を提案 | 約 3.5 h(API) | **済・不採用(決定 107)**: ループの正解はどの変種でも argmax にならず、偽陽性を減らす変種は k2 と `write_until` を消す。v7 は提案しない。ループは探索(測って返す)で拾う路線のまま |
...
| 16 | 目的は基準の上の +α。対象は何でもよく、小さくても再現する効果は捨てない。MDE は A/A から決める(決定 105)。確定: 集計 A/A の半幅 × 2、下限 1%、ケース別の主張はケース別 A/A(決定 106) | 2026-09-24「ノイズでなければ入れる」 |
...
- **MDE v2 での事後の読み直し(決定 105、`results.md` §171、再計測なし)**: 規則 `max(2 × 登録 A/A の最悪 per-case 半幅, 1%)`、対象ごとに凍結。v2 MDE は zopfli 訓練 1.838% / holdout 1.00%、hintbench 3.752%(固定クラスなら 3.176%)、jaq 3.26%。**下がるのは zopfli だけ**(hintbench は k8、jaq は objsearch の A/A で 3% より上がる)。zopfli の組み合わせは訓練 +1.811% / +2.063%、holdout +1.77% で、ラウンドバッチが 0.027 pt 足りない境界例。`squeeze.rs:325 unroll_count_8` は訓練平坦(+1.26 / +1.29)、holdout +1.33% は超える。有害は 2 → 5(関数属性 3 本、−2.0〜−2.7%)。hintbench は凍結で 4 本が変わる(k3 ループ `unroll_count_2` が効果に、3 本が有害に)。1〜3% 帯(k5 幅 16 +2.65% など)は v2 でも平坦。jaq は `write_until inline_never` が有害から外れる。zopfli の `h` を holdout パネルか集計 A/A で登録すれば MDE 1.00% で組み合わせと 325 は 3 脚とも効果(どれを登録するかはオーナー判断)。**driver の床は 0.03 のまま**(`jev_search.py` の凍結 `--mde`、`bench.py` 350 行)で、v2 の計測の前に直す(未実施)。
- **MDE v2 の確定(決定 106、`results.md` §172、オーナー判断 2026-09-24「おすすめでなおして」)**: MDE = `max(2 × 登録 A/A パネルの集計(幾何平均)半幅, 1%)`、ケース別の主張はそのケースの A/A 半幅 × 2(下限 1%)。3% の方針下限(決定 27)は撤回。確定値: zopfli 訓練 1.00%(集計 0.414% → 0.83% → 下限)/ holdout 1.00%、hintbench 集計 1.00%(§162、カーネル別 1.00〜3.18%)、jaq 1.13%(§140 の aa 0.564%。バッチ内半幅は過信気味なのでノイズの下限)。事後の読みで zopfli の組み合わせ(+1.81 / +2.06 / holdout +1.77%)と `squeeze.rs:325 unroll_count_8`(+1.26 / +1.29 / holdout +1.33%)が 3 脚とも超える = **再現性のある +α(事後と明記)**。driver は `[evaluation] mde_floor`(0.01)/ `mde_from`(`"aggregate"`)を持ち、`rounds.jsonl` と manifest に `mde_rule` / `mde_floor` / `mde_from` を記録(toy で検証、§172)。hintbench の Jev 見出し(Exp4〜6)は再採点しない。1〜2% の主張には n≥3(決定 96)。

- **Prompt study 2(決定 107、2026-09-24 午後、API のみ)**: v6 の一発要求(B0)と一律な 14 通りの聞き方 × 3 回、さらに偽陽性を減らす 5 通り。**ループの正解(k3 unroll 4、k5 / k8 幅 16、zopfli 325 unroll 8)はどの聞き方でも一度も選ばれない**(P 最大 0.19、他は 0.05 以下)。逆問いでは Jev はそれらを「最も有害」とみなす。関数側は頑健(hintbench 7/8、jaq で `write_until` を当てる)。偽陽性を減らす変更はすべて k2 の `inline(always)`(+67.8%)も消す。v7 は凍結しない。`results.md` §173〜§175、`docs/experiments/jev-prompt-study-2/`。
- **Marks study(決定 108、2026-09-24 夜、API のみ)**: perf 表の関数(reach 1% 以上、C 除外、アルファベット順)を Jev に一律に見せ、マークを選ばせた(Q1 Choice、Q2 Score、Q3 = 機械規則 + 灰色帯だけ Jev、各 3 回)。perf の割合だけ(v1)では jaq の Claude との一致 0.53 で不合格。命令数・ループ数・ケース別 reach などのバイナリの事実を足した v2 では **Q1 と Q3 が jaq 0.67 / zopfli 1.00 で事前登録の基準を通る**。ただし oracle の正解の被覆は機械規則(top-N reach / self)と同点の 1/2 で、`write_until`(reach 順位 10)は取るが `Val::hash`(reach 順位 83)は取らない。`--marks-by jev`(混成 Q3 の形)は設計のみ提案。79 要求、喪失 0、$0。`results.md` §176〜§177、`docs/experiments/jev-marks-study/`。
- **マークの規則化(決定 109、2026-09-25、API のみ)**: オーナーの決定でマーク選定を「規則の種(訓練 reach 上位 N ∪ self 上位 N)∪ 灰色帯だけ Jev」に固定し、`scripts/target_marks.py` を実装。jaq 34 行(種 26 + 灰色帯 85 問中 8、Claude の 15 本と正解 2 つを全部含む)、zopfli 14 行(種 10 + 灰色帯 14 問中 4、Claude の 6 本を全部含む)、hintbench 8 行(要求 0)。driver は `--marks` 省略時に生成済みファイルを使うか 1 回だけ生成し、manifest に由来を記録する。perf の表が無ければ準備として自動で取る(未実行)。`results.md` §179、`targets/*/jev-marks.jev.{txt,rationale.md}`、`README.md`(新規)。
...
| 7 | jaq が薄ければ記事の主対象を zopfli に(オーナー判断) | 「効果が出る対象を探して回る」のではなく、既に測った 2 対象からの選択 | — | **要判断**(決定 101)。事実: jaq はフラット(関数属性 oracle の組み合わせ +0.2%、決定 91)。zopfli は v1(MDE 3%)ではフラット(oracle の天井 +1.8%、Jev・ランダムとも採用 0、決定 100・101)。**確定した MDE v2(決定 106、集計 A/A × 2、下限 1%。zopfli は 1.00%)の下で、zopfli には再現性のある +1.8% の α がある(事後の読み)**: 組み合わせ(325 + 563 の unroll 8)が訓練 +1.81% / 確認 +2.06% / holdout +1.77%、`squeeze.rs:325 unroll_count_8` 単独が +1.26 / +1.29 / +1.33%で、3 脚とも 1.00% を超える。事前登録どおりの結果ではないので「事後」と明記する(v2 の事前登録で再計測すれば事後の注記が外れる)。正の結果(事前登録どおり)は hintbench の +7.4〜7.8%(決定 93)だけ。主対象の選択はオーナー判断 |
...
| 11 | マーク選定を規則 + 灰色帯 Jev で行う(`scripts/target_marks.py`、`target_sites.sh marks`、driver の `--marks` 省略時の既定) | パイプラインから手作業の点を無くす(オーナーの問い、2026-09-24) | 実装半日 | **済(決定 109)**: 種 = 訓練 reach 上位 N ∪ 訓練 self 上位 N、灰色帯(1% 以上の残り)だけ Jev の Choice 3 回・中央値 P ≥ 0.5。API 検証(`results.md` §179): jaq 34 行(凍結 15/15、正解 2/2 はどちらも種)、zopfli 14 行(凍結 6/6)、hintbench 8 行(要求 0)。9 要求、$0。出力は `jev-marks.jev.txt`(凍結の `jev-marks.txt` は書かない。置き換えは対象ごとにオーナー判断)。perf の表が無い新対象では準備として自動で profile を取る経路を実装(未実行、スタブで単体テスト)。決定 108 の `--marks-by jev`(Jev だけで選ぶ形)は採らない |
...
- **MDE の床は走行の時期で違う(決定 106)**: 決定 106 より前の走行(`rounds.jsonl` / manifest に `mde_rule` が無い)は `max(2 × 最悪 per-case 半幅, 3%)`、以後は既定で `max(2 × 集計 A/A 半幅, 1%)`(`mde_rule: "v2-aggregate"`)。旧走行と比べるときは `--mde-from worst_case --mde-floor 0.03` で旧規則を再現できる(記録済みの `mde` とビット一致、§172)。state 文の `{mde}` も「3%」から「1%」(凍結 `mde` があればその値)に変わる。hintbench を v2 で走らせるときは `--mde-case` に §162 のカーネル別 MDE を凍結する。

## 7. ファイル地図

```
SPEC.ja.md                 仕様(v0.5 + 決定 66〜106 反映)
AGENTS.md                  規約(英語)
HANDOFF.ja.md              この文書
results.md                 全計測(追記のみ、18 章超)
docs/decisions.ja.md       知見と判断(106 項目)

...
| 11 | マーク選定を Jev に任せる(`--marks-by jev`): perf + `inline_structure.py` の表(v2)から、明確な行は機械規則、灰色帯だけ Jev の Choice(3 回・中央値)で `jev-marks.txt` を作る | パイプラインから手作業の点を無くす(オーナーの問い、2026-09-24) | 実装半日 | **設計のみ提案(決定 108)**: API 実験では事前登録の基準を通った(v2、jaq 0.67 / zopfli 1.00)が、oracle の正解の被覆は機械規則と同点。実装・採用はオーナー判断 |

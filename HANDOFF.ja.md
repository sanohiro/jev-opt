...
この文書は、次に作業する人・エージェント(Claude でも Codex でも)が、経緯と現状と予定と作法を全部引き継げるように書いたもの。規約の短縮版は `AGENTS.md`(英語)。仕様は `SPEC.ja.md`、知見と判断の履歴は `docs/decisions.ja.md`(105 項目、番号で参照)、数値は `results.md`(章番号で参照、§171 まで)。この 3 つが正本で、本文書はそこへの案内図。
...
| 11 | マーク選定を Jev に任せる(`--marks-by jev`): perf + `inline_structure.py` の表(v2)から、明確な行は機械規則、灰色帯だけ Jev の Choice(3 回・中央値)で `jev-marks.txt` を作る | パイプラインから手作業の点を無くす(オーナーの問い、2026-09-24) | 実装半日 | **設計のみ提案(決定 108)**: API 実験では事前登録の基準を通った(v2、jaq 0.67 / zopfli 1.00)が、oracle の正解の被覆は機械規則と同点。実装・採用はオーナー判断。phase 3(判定行 M1・ケース別 M2・適用条件文 M3 とその組み合わせ、§178)は `Val::hash` を拾うがマークを 47〜109 本に膨らませ、事前登録の救済基準を満たさない。文面は v2 のまま |

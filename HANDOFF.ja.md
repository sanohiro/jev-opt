...
| 16 | 目的は基準の上の +α。対象は何でもよく、小さくても再現する効果は捨てない。MDE は A/A から決める(決定 105) | 2026-09-24「ノイズでなければ入れる」 |
...
| 7 | jaq が薄ければ記事の主対象を zopfli に(オーナー判断) | 「効果が出る対象を探して回る」のではなく、既に測った 2 対象からの選択 | — | **要判断**(決定 101)。事実: jaq はフラット(関数属性 oracle の組み合わせ +0.2%、決定 91)。zopfli は v1(MDE 3%)ではフラット(oracle の天井 +1.8%、Jev・ランダムとも採用 0、決定 100・101)。**MDE v2 での事後の読み直し(決定 105、§171)では、zopfli の組み合わせ +1.8%(訓練 +1.811% / +2.063%、holdout +1.77%)が確認バッチと holdout で v2 MDE を超える再現する α の候補**。ただし v2 の文言(訓練 MDE 1.838%)ではラウンドバッチが 0.027 pt 足りない境界例で、zopfli の `h` を holdout パネルか集計 A/A で登録すれば(MDE 1.00%)3 脚とも効果。事後なので「確認済みの α」と呼ぶには v2 の事前登録での再計測が要る。正の結果(事前登録どおり)は hintbench の +7.4〜7.8%(決定 93)だけ。主対象の選択と zopfli の `h` の登録はオーナー判断 |
